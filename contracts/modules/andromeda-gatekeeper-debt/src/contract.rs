...
use andromeda_modules::gatekeeper_common::LEGACY_OWNER;
#[cfg(not(feature = "library"))]
use cosmwasm_std::{
    to_binary, entry_point, Addr, BankMsg, Binary, Coin, CosmosMsg, Deps, DepsMut, Env, MessageInfo, Response,
    StakingMsg, StdError, StdResult, Uint128, WasmMsg,
};
use cosmwasm_std::{Api, Order, ensure};
...
use crate::state::{ObiProxyContract, State, STATE};
...
const CONTRACT_NAME: &str = "crates.io:andromeda-gatekeeper-debt";
...
pub fn instantiate(
    ...
    LEGACY_OWNER.save(deps.storage, &msg.owner)?;
        ...
        Ok((can_spend, spend_limit_reduction, repay_msg)) => {
            if !can_spend.can_spend {
                ...
                if let Some(spend_limit_reduction) = spend_limit_reduction {
                    ...
                    this_permissioned_address.reduce_limit_direct(spend_limit_reduction.coin)?;
                }
...
pub fn query(&self, deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::PermissionedAddresss {} => to_binary(&self.query_permissioned_addresses(deps)?),
        QueryMsg::CanSpend {
            sender,
            funds,
            msgs,
...
pub fn query_permissioned_addresses(&self, deps: Deps) -> StdResult<PermissionedAddresssResponse> {
    let cfg = self.cfg.load(deps.storage)?;
