    ...
    pub fn check_spend_limits(
        ...
        let cached_result: Result<SourcedCoins, CustomError> =
            if this_wallet.should_reset_active(current_time) {
                println!("\x1b[3m\tRecurring spend limit resets.\x1b[0m");
                this_wallet
                    .check_spend_vec(
                        deps,
                        asset_unifier_contract_address.clone(),
                ...
                println!("\x1b[3m\tRecurring spend limit does not reset.\x1b[0m");
                this_wallet
                    .check_spend_vec(
                        deps,
                        asset_unifier_contract_address.clone(),
        ...
        match cached_result {
            Err(e) => {
                if this_wallet.should_reset_beneficiary(current_time) {
                    println!("\x1b[3m\tBeneficiary spend limit resets.\x1b[0m");
                    ...
                    println!("\x1b[3m\tBeneficiary spend limit does not reset.\x1b[0m");
                    return Ok(coin);
                }
                Err(e)
            }
